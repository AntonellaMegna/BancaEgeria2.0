...
public class DepServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
       ...
       Movimenti mov= new Movimenti("Deposito",(new BigDecimal(request.getParameter("amount"))), LocalDate.now(),LocalTime.now(),conto);
       conto.setAmount(conto.getAmount().add(new BigDecimal(request.getParameter("amount"))));
    
       if(service.Dao.AggiornaMov(mov, conto)== true){
      	  response.sendRedirect("deposito.jsp");
       }else{
    	 
    	 request.getSession().setAttribute("msg", "Deposito non riuscito");
